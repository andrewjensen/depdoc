...
fn resolve_relative_path_import(
    ...
    let mut resolved_path = PathBuf::from(&node.path_absolute);
    resolved_path.pop();
...
mod test {
    ...
    fn test_resolve_imports_same_directory() {
        ...
        nodes_by_path.insert(source_node.path_absolute.clone(), &source_node);
        nodes_by_path.insert(target_node.path_absolute.clone(), &target_node);

        let unresolved_imports = vec![UnresolvedImport {
            module_name: "./subdirectory/myOtherFile".to_string(),
        }];

        let resolved_imports = resolve_imports(unresolved_imports, &source_node, &nodes_by_path);
        assert_eq!(
            resolved_imports,
            vec![ResolvedImport::InternalImport {
                target_path: "/path/to/repo/subdirectory/myOtherFile.tsx".to_string(),
    ...
    fn test_resolve_imports_far_away_directory() {
        ...
        assert_eq!(
            resolved_imports,
            vec![ResolvedImport::InternalImport {
                target_path: "/path/to/repo/feature-b/subfeature/myOtherFile.tsx".to_string(),
                target_node_id: "c319b51d-2f58-41aa-96f5-fbe35bb227b4".to_string()
            }]
        );
    }
